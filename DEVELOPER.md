...
4. **`FeedProvider`**: Üç upstream provider'a (`SubscriptionProvider`, `SettingsProvider`, `BookmarkProvider`) bağımlıdır. Bunlardan birisi değiştiğinde reaktif olarak tetiklenerek arama, kategori filtreleme, keyword engelleme ve okundu/okunmadı durumlarını birleşik pipeline (veri hattı) üzerinden süzüp arayüze sunar.
...
*   **Sınırlandırılmış Eşzamanlılık (Semaphore)**: Çok sayıda akışa abone olunduğunda cihazın ağ kuyruğunu tıkamamak için `_Semaphore` yapısıyla eşzamanlı HTTP istek sayısı **en fazla 5** ile sınırlandırılmıştır.
...
1.  **Isolate Tabanlı JSON İşlemleri**: Büyük boyutlu makale listelerinin JSON serileştirme/kod çözme işlemleri ana arayüz thread'ini (UI Thread) bloke edip donmalara (jank) sebep olmaması için `compute()` fonksiyonu aracılığıyla arka plandaki Dart izolatlarına (Background Isolates) devredilmiştir.
...
## 🔍 8. Kod Tabanındaki Bilinen Farklılıklar (Discrepancies)
